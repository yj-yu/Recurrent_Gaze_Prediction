...
class SaliencyModel(ModelBase):
    ...
    @staticmethod
    def create_shallownet(images, scope=None, net=None):
        """
        Args:
        ...
        with tf.variable_scope(scope or 'ShallowNet'):
            # CONV
            net['conv1'] = convolution2d(images, 32,
            ...
            net['conv2'] = convolution2d(net['pool1'], 64,
            ...
            net['conv3'] = convolution2d(net['pool2'], 64,
                                        kernel_size=(3, 3), stride=(1, 1), padding='VALID',
                                        activation_fn=None,#tf.nn.relu,
                                        weight_init=initializers.xavier_initializer_conv2d(uniform=True),
                                        bias_init=tf.constant_initializer(0.0),
            ...
            net['fc1'] = fully_connected(pool3_flat, 4802,
                                    activation_fn=None,#tf.nn.relu,
            ...
            '''
            net['fc2'] = fully_connected(net['max_out'], 7203,
            ...
            fc2_slice1, fc2_slice2, fc2_slice3 = tf.split(1, 3, net['fc2'], name='fc2_slice')
            net['max_out2'] = tf.maximum(fc2_slice1, fc2_slice2, name='fc2_maxout')
            net['max_out2'] = tf.maximum(fc2_slice3, net['max_out2'], name='fc2_maxout2')
            '''
            net['fc2'] = fully_connected(net['max_out'], 4802 ,
                                        activation_fn=None, # no relu here
                                        weight_init=initializers.xavier_initializer(uniform=True),
                                        bias_init=tf.constant_initializer(0.0),
                                        weight_collections=['MODEL_VARS'], bias_collections=['MODEL_VARS'],
    ...
    def build_model(self):
        ...
        # saliency maps (GT)
        self.saliencymaps_gt = tf.placeholder(tf.float32, shape=(None, 49, 49))
        ...
        self.reg_loss = 1e-6 * sum([tf.nn.l2_loss(t) for t in tf.get_collection('MODEL_VARS')])
        self.target_loss = 2.0 * tf.nn.l2_loss(self.saliency_output - self.saliencymaps_gt) / (49 * 49)
        ...
        _add_image_summary('saliency_maps_pred_original',
                           tf.reshape(self.saliency_output, [-1, 49, 49, 1]))
        _add_image_summary('saliency_maps_pred_norm',
                           tf.reshape(tf_normalize_map(self.saliency_output), [-1, 49, 49, 1]))
    ...
    def evaluate(self, dataset):
        ...
        for v in range(num_steps):
            ...
            saliency_output = saliency_output.reshape(-1, 49, 49)
